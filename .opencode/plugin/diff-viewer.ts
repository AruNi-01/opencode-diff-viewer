...
export const DiffViewerPlugin: Plugin = async ({ project, client, $, directory, worktree }) => {
  ...
  const installLumen = async (): Promise<{ success: boolean; method?: string; error?: string }> => {
    await client.app.log({
      service: "diff-viewer",
      level: "info",
      message: "lumen not found, attempting auto-install...",
    })
    ...
    if (await isCargoInstalled()) {
      try {
        ...
        await client.app.log({
          service: "diff-viewer",
          level: "warn",
          message: `cargo install failed: ${e}`,
        })
      }
    }
  ...
  const ensureLumenInstalled = async (): Promise<{ installed: boolean; message?: string }> => {
    ...
    if (result.success) {
      await client.app.log({
        service: "diff-viewer",
        level: "info",
        message: `lumen installed successfully via ${result.method}`,
      })
  ...
  if (initResult.message) {
    await client.app.log({
      service: "diff-viewer",
      level: initResult.installed ? "info" : "error",
  ...
  return {
    ...
    "file.edited": async ({ event }) => {
      await client.app.log({
        service: "diff-viewer",
        level: "debug",
        message: `File edited: ${event.path}`,
      })
    },
